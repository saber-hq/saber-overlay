...
let
  ...
  tool-anchor = if (builtins.hasAttr "anchor" buildTools) then buildTools.anchor else pkgs.anchor;
  tool-solana = if (builtins.hasAttr "solana" buildTools) then buildTools.solana else pkgs.solana-basic;
